...
export const cartSlice = createSlice({
  ...
  reducers: {
    ...
    addItem: (state, action: PayloadAction<Game>) => {
      ...
      const gameInCart = state.gameStorage.find(
      ...
      state.totalAmount = state.gameStorage.reduce(
        (count, game) => count + game.itemAmount,
        0
      );
    ...
    removeItem: (state, action: PayloadAction<Game>) => {
      ...
      state.gameStorage = state.gameStorage.filter(
        (cartItem) => cartItem.id !== action.payload.id
      );
