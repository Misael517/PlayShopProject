import styles from './ImmortalsOfAveum.module.css';
                ...
                <section className={styles.section1}>
                    ...
                    <div className={styles.buyingSection}>
                        ...
                        <div className={styles.gamesInfo}>
                            ...
                            <div className={styles.gamesPrice}>
                                <p><span className={jsonData[33].isOnSale ? styles.discountColor : ''}>{jsonData[33].isOnSale ? `-${jsonData[33].discount}%` : ''}</span></p>
                                <p><span className={jsonData[33].isOnSale ? styles.strikeThrough : ''}>{jsonData[33].isOnSale ? `${jsonData[33].price}%` : ''}</span></p>
                                <p style={{ textAlign: 'center' }}>{jsonData[33].isOnSale ? `$${jsonData[33].actualPrice}` : (jsonData[33].coomingSoon ? '...' : `$${jsonData[33].price}`)}</p>
                            </div>
                        </div>
                    ...
                    <div className={styles.detailsContainer}>
                        <div>
                            ...
                            <p className={styles.detailsContent}>{jsonData[33].Platforms}</p>
                        </div>

                        <div>
                            <h3 className={styles.detailsTitle}>Publisher</h3>
                            <p className={styles.detailsContent}>{jsonData[33].Publisher}</p>
                        </div>

                        <div>
                            <h3 className={styles.detailsTitle}>Video Game Genre</h3>
                            <p className={styles.detailsContent}>{jsonData[33].Genre}</p>
                        </div>
                    </div>
                </section>
            </main>
...
export default ImmortalsOfAveum