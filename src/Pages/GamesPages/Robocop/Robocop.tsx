import styles from './Robocop.module.css';
                ...
                <section className={styles.section1}>
                    ...
                    <div className={styles.buyingSection}>
                        ...
                        <div className={styles.gamesInfo}>
                            ...
                            <div className={styles.gamesPrice}>
                                <p><span className={jsonData[31].isOnSale ? styles.discountColor : ''}>{jsonData[31].isOnSale ? `-${jsonData[31].discount}%` : ''}</span></p>
                                <p><span className={jsonData[31].isOnSale ? styles.strikeThrough : ''}>{jsonData[31].isOnSale ? `${jsonData[31].price}%` : ''}</span></p>
                                <p style={{ textAlign: 'center' }}>{jsonData[31].isOnSale ? `$${jsonData[31].actualPrice}` : (jsonData[31].coomingSoon ? '...' : `$${jsonData[31].price}`)}</p>
                            </div>
                        </div>
                    ...
                    <div className={styles.detailsContainer}>
                        <div>
                            ...
                            <p className={styles.detailsContent}>{jsonData[31].Platforms}</p>
                        </div>

                        <div>
                            <h3 className={styles.detailsTitle}>Publisher</h3>
                            <p className={styles.detailsContent}>{jsonData[31].Publisher}</p>
                        </div>

                        <div>
                            <h3 className={styles.detailsTitle}>Video Game Genre</h3>
                            <p className={styles.detailsContent}>{jsonData[31].Genre}</p>
                        </div>
                    </div>
                </section>
            </main>
